...
export function CommentSection({ taskId, comments, projectId, onCommentAdded }: CommentSectionProps) {
  const [newComment, setNewComment] = useState('');
  const [authorEmail, setAuthorEmail] = useState('');
  
  const [addComment, { loading }] = useMutation(ADD_TASK_COMMENT, {
    refetchQueries: [
      { query: GET_TASK_COMMENTS, variables: { taskId } },
    ],
    onCompleted: () => {
      setNewComment('');
      if (onCommentAdded) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3 max-h-96 overflow-y-auto">
        {comments.length === 0 ? (
          <p className="text-gray-500 text-sm py-4 text-center">No comments yet</p>
        ) : (
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <Textarea
          ...
          required
        />
        <Button type="submit" disabled={loading || !newComment.trim() || !authorEmail.trim()}>
